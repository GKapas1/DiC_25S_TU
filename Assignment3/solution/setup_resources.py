...
def setup_ssm():
    ...
    def put_if_missing(name, value):
        if name in param_names:
            ...
            run(f"awslocal ssm put-parameter --name {name} --type String --value {value}")
...
def package_and_deploy_lambdas():
    for name in LAMBDA_FUNCTIONS:
        path = f"lambdas/{name}"
        zip_path = f"{path}/lambda.zip"

        run(f"rm -rf {path}/package {zip_path}")
        run(f"mkdir -p {path}/package")
        run(f"pip3 install -r {path}/requirements.txt -t {path}/package "
            f"--platform manylinux2014_x86_64 --only-binary=:all: --implementation cp --python-version 3.11 --abi cp311")
        run(f"cp {path}/handler.py {path}/package/")
        run(f"cd {path}/package && zip -r ../lambda.zip .")

        #deploying to LocalStack
        run(f"""awslocal lambda create-function \
            --function-name {name} \
            --runtime python3.11 \
...
def setup_s3_trigger():
    ...
    notification_json = '''{
      "LambdaFunctionConfigurations": [{
        "LambdaFunctionArn": "arn:aws:lambda:us-east-1:000000000000:function:preprocessing",
